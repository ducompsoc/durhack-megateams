...
export async function makeMegateamsApiRequest(endpoint: string, options?: RequestInit) {
...
export async function fetchMegateamsApi(
  endpoint: string,
  options?: RequestInit
) {
  const request = await makeMegateamsApiRequest(endpoint, options);
