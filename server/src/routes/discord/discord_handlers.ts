...
class DiscordHandlers {
  ...
  async handleRedirect(request: Request, response: Response) {
    ...
    const { code, state } = DiscordHandlers.discord_access_code_schema.parse(
      request.query
    );
    ...
    const encoded_access_code_exchange_payload = new URLSearchParams(
      access_code_exchange_payload
    );

    const discord_access_token_response = await fetch(
    ...
    if (!discord_access_token_response.ok) {
      throw new createHttpError.BadGateway(
        "Couldn't exchange access code for access token."
      );
    }
    ...
    const discord_profile_response = await fetch(
      `${discordApiBase}/oauth2/@me`,
      {
        headers: {
    ...
    if (!discord_profile_response.ok) {
      throw new createHttpError.BadGateway(
        "Failed to read your Discord profile."
      );
    }
    ...
    await fetch(
      `${discordApiBase}/channels/${team.discord_channel_id}/permissions/${discord_user_id}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bot ${config.get("discord.botToken")}`,
