  ...
  async getMegateamsList(req: Request, res: Response): Promise<void> {
    const result = await Megateam.findAll({
      attributes: [
        ...
        [
            Sequelize.fn("count", Sequelize.col("areas.team.members.user_id")),
            "members"
        ]
      ],
      include: [
    ...
    const most_members = Math.max(...result.map( megateam => MegateamHandlers.numberParser.parse(megateam.toJSON().members) ));
